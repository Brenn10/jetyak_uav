...
void util::rpy_from_quat(const geometry_msgs::Quaternion &orientation,
...
template <typename T> T util::clip(T x, T low, T high) {
  return std::max(std::min(high, x), low);
}

void util::rotate_vector(double x, double y, double theta, double &xp,
                         double &yp) {
  xp = (x * std::cos(theta) - y * std::sin(theta));
  yp = (x * std::sin(theta) + y * std::cos(theta));
}
void util::inverse_pose(const geometry_msgs::Pose &in,
