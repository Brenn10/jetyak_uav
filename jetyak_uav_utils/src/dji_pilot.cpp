...
dji_pilot::dji_pilot(ros::NodeHandle &nh) {
  ...
  controlPub = nh.advertise<sensor_msgs::Joy>(
  ...
  taskServ = nh.serviceClient<dji_sdk::DroneTaskControl>(
      "/dji_sdk/drone_task_control");

  sdkCtrlAuthorityServ = nh.serviceClient<dji_sdk::SDKControlAuthority>(
  ...
  propServServer =
      nh.advertiseService("prop_enable", &dji_pilot::propServCallback, this);
  takeoffServServer =
      nh.advertiseService("takeoff", &dji_pilot::takeoffServCallback, this);
  landServServer =
      nh.advertiseService("land", &dji_pilot::landServCallback, this);
...
void dji_pilot::rcCallback(const sensor_msgs::Joy::ConstPtr &msg) {
  ...
  else if ((msg->axes[4] == modeFlag && msg->axes[5] != pilotFlag &&
            autopilotOn) ||
  ...
  if (msg->axes[4] == modeFlag && autopilotOn) {
    if (std::abs(msg->axes[0]) > rcStickThresh ||
        std::abs(msg->axes[1]) > rcStickThresh ||
        std::abs(msg->axes[2]) > rcStickThresh ||
        std::abs(msg->axes[3]) > rcStickThresh) {
...
sensor_msgs::Joy dji_pilot::adaptiveClipping(sensor_msgs::Joy msg) {
  ...
  if (((int)msg.axes[4] & DJISDK::HORIZONTAL_BODY) == DJISDK::HORIZONTAL_BODY) {
  ...
  // Vertical Logic
  if (((int)msg.axes[4] & DJISDK::VERTICAL_THRUST) == DJISDK::VERTICAL_THRUST)
